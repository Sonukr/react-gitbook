...
anObject.bye = 'goodbye';
```

is valid! This, however:
...
is not, because we reassigned the value of `hi`.

Generally, if you need to declare a variable whose value will _not_ change, declare it as a const.

`let`, like `var`, can be reassigned as much as you like. Why use `let` then, why not just use `var`? Because `let` is block-scoped, which is more predictable and straightforward than `var`'s scoping.  
